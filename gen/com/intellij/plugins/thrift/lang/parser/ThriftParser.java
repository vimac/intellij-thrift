...
public class ThriftParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // std_namespace | php_namespace | xsd_namespace | nova_namespace
  ...
  //                      'cocoa' | 'csharp' | 'c_glib' | 'go' | 'php' | 'delphi' | 'nova'
  ...
  /* ********************************************************** */
  // 'nova_namespace' Literal
  static boolean nova_namespace(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "nova_namespace")) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_);
    r = consumeToken(b, "nova_namespace");
    p = r; // pin = 1
    r = r && consumeToken(b, LITERAL);
    exit_section_(b, l, m, r, p, null);
    return r || p;
  ...
  private static boolean topLevelRecover_0(PsiBuilder b, int l) {
    ...
    if (!r) r = consumeToken(b, "nova_namespace");
    exit_section_(b, m, null, r);
    return r;
  }
